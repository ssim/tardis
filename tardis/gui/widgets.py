...
class LineInfo(QtGui.QDialog):
    ...
    def get_data(self, wavelength_start, wavelength_end):
        ...
        self.last_line_in, self.last_line_out = (
            self.parent.model.plasma.lines.ix[last_line_in_ids], 
            self.parent.model.plasma.lines.ix[last_line_out_ids])
    ...
    def get_transition_table(self, lines, atom, ion):
        ...
        for key, value in transitions.items():
            transitions_parsed.append("%d-%d (%.2f A)" % (key[0], key[1], 
                self.parent.model.plasma.atomic_data.lines.ix[value[0]]['wavelength']))
...
class LineInteractionTables(QtGui.QWidget):
    ...
    def __init__(self, line_interaction_analysis, atom_data, lines_data, description, 
        ...
        temp = line_interaction_analysis.last_line_in_table.fillna(0)
        line_interaction_species_group = \
        temp.groupby(['atomic_number', 
            'ion_number'])
        self.species_selected = sorted(
            line_interaction_species_group.groups.keys())
        #self.species_selected = self.species_selected[1:]
        ...
        species_abundances = (
            ...
            line_interaction_analysis.last_line_in_table.wavelength.count()).astype(float).tolist()
